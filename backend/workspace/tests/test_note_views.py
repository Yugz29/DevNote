...
class NoteViewTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="notetestuser", email="note@test.com", password="TestPass123!"
        )
        ...
        self.note = Note.objects.create(
            title="Test Note", content="This is a test note.", project=self.project
        )
    ...
    def test_create_note(self):
        ...
        data = {"title": "New Note", "content": "A new test Note"}
        response = self.client.post(
            f"/api/projects/{self.project.id}/notes/", data, format="json"
        )
        ...
        self.assertTrue(
            Note.objects.filter(title="New Note", project=self.project).exists()
        )

    def test_create_note_unauthenticated(self):
        """Test creating note when unauthenticated"""
        self.client.force_authenticate(user=None)
        data = {"title": "Unauthorized Note", "content": "Unauthorized Note content"}
        response = self.client.post(
            f"/api/projects/{self.project.id}/notes/", data, format="json"
        )
    ...
    def test_update_note(self):
        ...
        data = {"title": "Updated title Note", "content": "Updated content Note"}
    ...
    def test_user_isolation(self):
        ...
        response = self.client.post(
            f"/api/projects/{other_project.id}/notes/", data, format="json"
        )
    ...
    def test_duplicate_note(self):
        ...
        response = self.client.post(
            f"/api/notes/{self.note.id}/duplicate/", format="json"
        )
    ...
    def test_duplicate_note_keeps_folder(self):
        ...
        response = self.client.post(f"/api/notes/{note.id}/duplicate/", format="json")
    ...
    def test_duplicate_note_numbers_further_copies(self):
        ...
        first = self.client.post(f"/api/notes/{self.note.id}/duplicate/", format="json")
    ...
    def test_duplicate_note_user_isolation(self):
        ...
        response = self.client.post(
            f"/api/notes/{other_note.id}/duplicate/", format="json"
        )
    ...
    def test_pin_note_through_patch(self):
        ...
        response = self.client.patch(
            f"/api/notes/{self.note.id}/", {"is_pinned": True}, format="json"
        )
        ...
        response = self.client.patch(
            f"/api/notes/{self.note.id}/", {"is_pinned": False}, format="json"
        )
    ...
    def test_pinning_leaves_the_note_in_place(self):
        ...
        response = self.client.patch(
            f"/api/notes/{note.id}/", {"is_pinned": True}, format="json"
        )
    ...
    def test_pinning_a_foreign_note_denied(self):
        ...
        response = self.client.patch(
            f"/api/notes/{other_note.id}/", {"is_pinned": True}, format="json"
        )
...
class ProjectPinnedViewTest(APITestCase):
    ...
    def test_pinned_is_scoped_to_the_project(self):
        ...
        Note.objects.create(title="Elsewhere", project=other_project, is_pinned=True)
        mine = Note.objects.create(title="Mine", project=self.project, is_pinned=True)
