...
class TodoListModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="listtestuser", email="list@test.com", password="TestPass123!"
        )
    ...
    def test_lists_are_ordered_by_name_under_the_permanent_one(self):
        ...
        self.assertEqual(names, ["Top priorities", "Backlog", "Later", "Sprint 1"])
    ...
    def test_todo_can_belong_to_a_list(self):
        ...
        todo_list = TodoList.objects.create(name="Sprint 1", project=self.project)
        todo = TODO.objects.create(title="Filed", project=self.project, list=todo_list)
    ...
    def test_ensure_permanent_returns_the_existing_one(self):
        ...
        existing = TodoList.objects.get(project=self.project, is_permanent=True)

        self.assertEqual(TodoList.ensure_permanent(self.project).id, existing.id)
        self.assertEqual(
            TodoList.objects.filter(project=self.project, is_permanent=True).count(), 1
        )

    def test_ensure_permanent_recreates_a_missing_one(self):
        """Test : the helper covers a project that somehow lost it"""
        TodoList.objects.filter(project=self.project, is_permanent=True).delete()
    ...
    def test_the_permanent_list_can_be_renamed(self):
        ...
        todo_list = TodoList.objects.get(project=self.project, is_permanent=True)
