...
from workspace.models import TODO, Project, TodoList
...
class TodoListViewTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="listdev", email="listdev@test.com", password="TestPass123!"
        )
        ...
        self.other_project = Project.objects.create(
            title="Foreign List Project", user=self.other_user
        )
    ...
    def test_rename_list(self):
        ...
        response = self.client.patch(
            f"/api/todo-lists/{self.todo_list.id}/", {"name": "Sprint 2"}, format="json"
        )
    ...
    def test_rename_to_an_existing_name_rejected(self):
        ...
        response = self.client.patch(
            f"/api/todo-lists/{self.todo_list.id}/", {"name": "Backlog"}, format="json"
        )
    ...
    def test_lists_of_a_foreign_project_denied(self):
        ...
        response = self.client.get(f"/api/projects/{self.other_project.id}/todo-lists/")
...
class TodoListFilterViewTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="filterdev", email="filterdev@test.com", password="TestPass123!"
        )
        self.client.force_authenticate(user=self.user)

        self.project = Project.objects.create(title="Filter Project", user=self.user)
    ...
    def test_unfiltered_listing_returns_every_todo(self):
        ...
        ids = {entry["id"] for entry in response.data["results"]}
        self.assertEqual(
            ids, {str(self.filed.id), str(self.other_filed.id), str(self.loose.id)}
        )
    ...
    def test_filter_by_null_list(self):
        ...
        response = self.client.get(f"/api/projects/{self.project.id}/todos/?list=null")
    ...
    def test_todo_is_unclassified_on_create(self):
        ...
        response = self.client.post(
            f"/api/projects/{self.project.id}/todos/", {"title": "Fresh"}, format="json"
        )
    ...
    def test_unclassify_todo_through_patch(self):
        ...
        response = self.client.patch(
            f"/api/todos/{self.filed.id}/", {"list": None}, format="json"
        )
    ...
    def test_filing_into_a_foreign_list_denied(self):
        ...
        foreign_list = TodoList.objects.create(name="Theirs", project=other_project)
