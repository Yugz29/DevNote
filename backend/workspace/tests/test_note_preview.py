...
class MarkdownToPlainTextTest(TestCase):
    ...
    def test_inline_code(self):
        self.assertEqual(
            markdown_to_plain_text("Run `npm run build` now."), "Run npm run build now."
        )
    ...
    def test_images_are_dropped(self):
        self.assertEqual(
            markdown_to_plain_text("Before ![alt](img.png) after"), "Before after"
        )
    ...
    def test_table(self):
        self.assertEqual(
            markdown_to_plain_text("| Key | Value |\n|---|---|\n| one | two |"),
            "Key Value one two",
        )
